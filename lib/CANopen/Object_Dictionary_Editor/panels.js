...
function g_codePanelInit(panelArgument){
	...
	if(panelArgument.substring(0, 1) == "<")
		var text = panelArgument;
	else
		eval("var text = " + panelArgument + ".toXMLString();");

	var texbox = document.getElementById('g_codePanelText');
	var button = document.getElementById('g_codePanelButton');

	if(panelArgument == "g_project"){
		text = "<?xml version=\"1.0\"?>\n" + text;
		texbox.readOnly = "";
		button.disabled = "";
	}
	else if(panelArgument.substring(0, 1) == "<"){
		texbox.readOnly = "";
		button.disabled = "";
	}
	else{
...
function g_codePanelUpdate(){
	if(!confirm("Are you sure, you want to update the project from manual editing?\nClick yes only, if you are sure, what you are doing!")) return;
	var text = document.getElementById('g_codePanelText').value;
	g_createProject(text);
}
...
function featureInit(panelArgument){
	...
	for(var i=field.getRowCount()-1; i>=0; i--) field.removeItemAt(i);
	//add new items
	if(el.length()) for each (var obj in el) field.appendChild(g_createListitem([obj.@index, obj.@indexMax, obj.@indexStep]));
		
	if(!document.getElementById("g_infoWindow").hidden)
		g_labelEdit("feature.curent", "Edit Feature Description");
}
...
function featureInsertNewObject(){
	...
	var tf3 = document.getElementById("feature_indexStep");
	var text1 = tf1.value;
	var text2 = tf2.value; if(!text2) text2 = text1;
	var text3 = tf3.value; if(!text3) text3 = "1";
	var n1 = parseInt("0x"+text1);
	var n2 = parseInt("0x"+text2);
	var n3 = parseInt("0x"+text3);
	...
	if(!(n3 >= 0x1 && n3 <= 0xF000)){
		alert("Index step must be between 1 and 0xF000!");
		return;
	}
	...
	if(n1==n2){
		...
		text2 = g_wordToHexString(n2);
		text3 = n3.toString(16).toUpperCase();
	}

	document.getElementById("feature_objects").appendChild(g_createListitem([text1, text2, text3]));
...
function featureUpdateOrInsertNew(insertNew){
	...
	field = document.getElementById("feature_objects");
		for(var i=0; i<field.getRowCount(); i++){
			...
			var indexMax = field.getItemAtIndex(i).firstChild.nextSibling.getAttribute("label")
			var indexStep = field.getItemAtIndex(i).lastChild.getAttribute("label")
			var ao = <associatedObject index={index}/>;
			if(indexMax){
				ao.@indexMax = indexMax;
				if(!indexStep) indexStep = "1";
				ao.@indexStep = indexStep;
			}
			newFeature += ao;
		}
...
function objectInit(panelArgument){
	...
	//TPDO detect COS flag, default and actual value
		document.getElementById("object_TPDOdetectCOS").checked = (curent.@TPDOdetectCOS.toString()=="true")?true:false;
	...
	if(!document.getElementById("g_infoWindow").hidden)
		g_labelEdit("object.curent", "Edit Object Description");
}
...
//enable/disable some fields, of object type has changed
	...
	switch(objectType){
		...
		default:
			...
			document.getElementById("object_TPDOdetectCOS").removeAttribute("disabled");
		...
		case "9 - Record":
			...
			f = document.getElementById("object_TPDOdetectCOS"); f.setAttribute("disabled", "true"); f.checked = false;
			f = document.getElementById("object_defaultValue"); f.value = ""; f.setAttribute("disabled", "true");
			f = document.getElementById("object_actualValue"); f.value = ""; f.setAttribute("disabled", "true");
	}
}
...
//function updates curent object or inserts new, if insertNew is true
	...
	if(objectType=="7" || objectType=="8"){
		...
		//TPDO detect COS
			field = document.getElementById("object_TPDOdetectCOS").checked;
			if(field){
				var err = "TPDO can not detect Change of State of the variable, if ";
		...
		document.getElementById("object_TPDOdetectCOS").checked = false;
	}
...
function subobjInit(panelArgument){
	...
	var subobj_TPDOdetectCOS = document.getElementById("subobj_TPDOdetectCOS");
	...
	if(subindex == "00"){
		...
		subobj_TPDOdetectCOS.setAttribute("checked", "false"); subobj_TPDOdetectCOS.setAttribute("disabled", "true");
		...
		if(parent.@accessFunctionName.length()){
			...
			curent.@defaultValue = subobj_defaultValue.value;
		}
	}
	else{
		...
		if(parentObjectType == "8"){
			...
			subobj_TPDOdetectCOS.setAttribute("checked", (parent.@TPDOdetectCOS.toString()=="true")?"true":"false");
			subobj_TPDOdetectCOS.setAttribute("disabled", "true");
		}
		else{
			...
			subobj_TPDOdetectCOS.removeAttribute("disabled"); subobj_TPDOdetectCOS.setAttribute("checked", (curent.@TPDOdetectCOS.toString()=="true")?"true":"false");
		}
		subobj_defaultValue.value = curent.@defaultValue;	subobj_defaultValue.removeAttribute("disabled");
	...
	if(!document.getElementById("g_infoWindow").hidden)
		g_labelEdit("subobj.parent", "Edit Object Description");
}
...
//function updates curent subobj
	...
	if(parentObjectType=="9" || subIndex=="00"){
		...
		//PDO mapping
			...
			newObject.@PDOmapping = PDOmapping;
		//TPDO detect COS
			field = document.getElementById("subobj_TPDOdetectCOS").checked;
			if(field){
				var err = "TPDO can not detect Change of State of the variable, if ";
				if(accessType=="wo"){alert(err+"Access Type is 'wo'"); return;}
				if(PDOmapping!="optional" && PDOmapping!="TPDO"){alert(err+"PDOmapping is 'no' or 'RPDO'"); return;}
